...
@Component({
  selector: 'app-contactus-item',
  imports: [ReactiveFormsModule],
  templateUrl: './contactus-item.component.html',
  styleUrl: './contactus-item.component.css'
})
export class ContactusItemComponent implements OnInit {
  ...
  // FormGroup para editar los datos de contacto
  ...
  idContactUs = computed(() => this.dataPage().pageID);
    ...
    if (this.selectedFile) {
      this.cloudinaryService.processImage(this.selectedFile).subscribe({
        next: (cloudinaryURL) => {
          ...
          this.updateContactUsWithImage(numericId, content ?? '', cloudinaryURL);
        },
        error: (error) => {
      ...
      this.updateContactUsWithImage(numericId, content ?? '', existingImage.toString());
    }
  }

  private updateContactUsWithImage(id: number, content: string, imageURL: string): void {
